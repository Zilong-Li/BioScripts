...
def _read_ids(theInVcf):
    ...
    vcfin = VCF(theInVcf)
    for v in vcfin:
        k = v.CHROM.replace("chr", "") + ':' + str(v.start+1)
        d[k] = v.REF + ":" + v.ALT[0]
    vcfin.close()
    return d
...
def main():
    ...
    parser.add_argument("ref", metavar="REF_VCF",
                        help="vcf file used as ref.")
    parser.add_argument("vcf", metavar="IN_VCF",
                        help="input vcf to be fixed")
    parser.add_argument("out", metavar="OUT_VCF",
                        help="output vcf")
